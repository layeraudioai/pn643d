...
int _SetMemoryPermission(void *buffer, int size, int permission)
{
	...
	const Result duplicateResult = svcDuplicateHandle(&currentHandle, 0xFFFF8001);
	if (R_FAILED(duplicateResult))
		return duplicateResult;

	const Result result = svcControlProcessMemory(currentHandle, (u32)buffer, 0, size, MEMOP_PROT, permission);
